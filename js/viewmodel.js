...
function NewSommelier(name) {                   // конструктор сомелье
  this.name = name;
  this.testPass = ko.observable(true);
}
...
function SommelierViewmMdel() {
  ...
  self.addSommelier = function() {              // метод создает нового сомелье
    ...
    self.sommeliers.push(new NewSommelier(self.newSommelierName()));
    self.newSommelierName('');
  };

  self.getWines = function(sommelier) {         // метод, обращаясь к условной бд,
    sommelier.testPass(false);                  // создает список винных бутылок для
